...
function closeUI() {
  $("#main-ui").fadeOut();
  $.post(`https://ValoriaAC/onCloseMenu`);
}
...
function getAdminStatus() {
  $.post(`https://ValoriaAC/getAdminStatus`);
}

function getAdminCoords() {
  $.post(`https://ValoriaAC/getPlayerCoords`);
}

function getPlayersData() {
  $.post(`https://ValoriaAC/getAllPlayersData`);
}

function getBanListData() {
  $.post(`https://ValoriaAC/getBanListData`);
}

function getAdminListData() {
  $.post(`https://ValoriaAC/getAdminListData`);
}

function getUnbanAccessData() {
  $.post(`https://ValoriaAC/getUnbanAccessData`);
}

function getWhitelistData() {
  $.post(`https://ValoriaAC/getWhitelistData`);
}
...
function doAction(actionName) {
  $.post(`https://ValoriaAC/${actionName}`);
...
function teleportToWaypoint() {
  $.post(`https://ValoriaAC/teleportToWaypoint`);
  closeUI();
}
...
function teleportToCoords() {
  ...
  if (xValue && yValue && zValue) {
    $.post(
      `https://ValoriaAC/teleportToCoords`,
      JSON.stringify({
        x: xValue,
        y: yValue,
        z: zValue,
...
function spawnVehicleForSelf() {
  ...
  if (vehicleName) {
    $.post(
      `https://ValoriaAC/spawnVehicleForSelf`,
      JSON.stringify({
...
function spawnVehicleOthers() {
  ...
  if (vehicleName && targetId) {
    $.post(
      `https://ValoriaAC/spawnVehicleOthers`,
      JSON.stringify({
        vehicleName: vehicleName,
        targetId: targetId,
      })
...
function unbanSelectedPlayer(banID) {
  $.post(
    `https://ValoriaAC/unbanSelectedPlayer`,
    JSON.stringify({
      banID: banID,
    })
  );
...
function removeSelectedAdmin(id) {
  $.post(
    `https://ValoriaAC/removeSelectedAdmin`,
    JSON.stringify({
      id: id,
    })
  );
...
function removeUnbanAccess(id) {
  $.post(
    `https://ValoriaAC/removeUnbanAccess`,
    JSON.stringify({
      id: id,
    })
  );
...
function removeWhitelistUser(id) {
  $.post(
    `https://ValoriaAC/removeWhitelistUser`,
    JSON.stringify({
      id: id,
    })
  );
